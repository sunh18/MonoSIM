...
from eval.rotate_iou import rotate_iou_gpu_eval
import pdb
...
def ignore_occluded(gt_anno):
    num_objects = len(gt_anno['name'])
    ignore_list = [False]*num_objects
    for i in range(0, num_objects):
        front_obj = gt_anno['bbox'][i]
        for j in range(0, num_objects):
            if i == j: 
                continue
            if gt_anno['location'][j][2] < gt_anno['location'][i][2]:
                continue
            back_obj = gt_anno['bbox'][j]
            if front_obj[0]<back_obj[0] and front_obj[1] < back_obj[1] and front_obj[2] > back_obj[2] and front_obj[3] > back_obj[3]:
                ignore_list[j] = True
    return ignore_list

def clean_data(gt_anno, dt_anno, current_class, difficulty, occluded_filter = True):
    ...
    num_valid_gt = 0
    occluded_count = 0 
    if occluded_filter == True:
        occluded_ignores = ignore_occluded(gt_anno)
    for i in range(num_gt):
        ...
        if occluded_ignores[i]==True:
            occluded_count += 1
        if occluded_filter == True and ignore == False and occluded_ignores[i]==True:
            ignore = True
    ...
    if compute_fp:
        ...
        if compute_aos:
            ...
            if tp > 0 or fp > 0:
                ...
                similarity = -1
    return tp, fp, fn, similarity, thresholds[:thresh_idx], detected_idx
...
def fused_compute_statistics(overlaps,
    ...
    for i in range(gt_nums.shape[0]):
        for t, thresh in enumerate(thresholds):
            ...
            tp, fp, fn, similarity, threshold, detected_idx= compute_statistics_jit(
                overlap,
                gt_data,
                dt_data,
                ignored_gt,
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, clean_data_func=clean_data):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        rets = clean_data_func(gt_annos[i], dt_annos[i], current_class, difficulty)
...
def eval_class(gt_annos,
    ...
    Args:
        ...
        num_parts: int. a parameter for fast calculate algorithm
        clean_data_func: func, Function to remove ground truth and detection boxes that are not used for evaluation
    Returns:
    ...
    tp_list = []
    fn_list = []
    tp_annos = {"dims":[], "locs":[], "ry":[]}
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty,
                                 clean_data_func=clean_data_func)
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    rets = compute_statistics_jit(
                        ...
                        thresh=0.0,
                        compute_fp=False)
                    tp, fp, fn, similarity, thresholds, detected_idx = rets
                    if len(detected_idx)!=0 and difficulty == 2:
                        print(detected_idx)
                        for idx in detected_idx:
                            tp_annos['dims'].append(gt_annos[i]['dimensions'][idx])
                            tp_annos['locs'].append(gt_annos[i]['location'][idx])
                            tp_annos['ry'].append(gt_annos[i]['rotation_y'][idx])
                    thresholdss += thresholds.tolist()
                    tp_list.append(tp)
                    fp_list.append(fp)
                    fn_list.append(fn)
                # if metric == 2 and difficulty == 2 and min_overlap == 0.7: 
                #     with open('output/eval/detections/left_left.pkl', 'wb') as f:
                #         pickle.dump(tp_annos, f)
...
def do_eval(gt_annos,
    ...
    ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 1,
                     min_overlaps, clean_data_func=clean_data_func)
...
def do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges,
                       compute_aos, clean_data_func=clean_data):
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, class_to_name, PR_detail_dict=None,
                             clean_data_func=clean_data):
