...
class WaymoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, conf, root, cache_folder=None):
        ...
        # use cache?
        if (cache_folder is not None) and os.path.exists(os.path.join(cache_folder, 'imdb_waymo_{}.pkl'.format(self.camera))):
        ...
        # self.loader = torch.utils.data.DataLoader(self, conf.batch_size, sampler=self.sampler, collate_fn=self.collate, num_workers=4)
