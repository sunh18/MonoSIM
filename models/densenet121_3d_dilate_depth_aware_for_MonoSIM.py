...
class RPN(nn.Module):
    ...
    def __init__(self, phase, base, conf):
        ...
        self.align_1 = nn.Sequential(
            nn.Conv2d(512, self.C_align_1, 1, ),
            nn.ReLU(inplace=True),
        )

        self.align_2 = nn.Sequential(
            nn.Conv2d(self.C_align_1, self.C_align_2, 1, ),
            nn.ReLU(inplace=True),
        )

        # outputs
        self.cls = nn.Conv2d(self.C_align_2, self.num_classes * self.num_anchors, 1, )
        # self.cls = nn.Conv2d(self.prop_feats[0].out_channels, self.num_classes * self.num_anchors, 1, )

        # bbox 2d
        self.bbox_x = nn.Conv2d(self.C_align_1, self.num_anchors, 1)
        self.bbox_y = nn.Conv2d(self.C_align_1, self.num_anchors, 1)
        self.bbox_w = nn.Conv2d(self.C_align_1, self.num_anchors, 1)
        self.bbox_h = nn.Conv2d(self.C_align_1, self.num_anchors, 1)

        # bbox 3d
        self.bbox_x3d = nn.Conv2d(self.C_align_1, self.num_anchors, 1)
        self.bbox_y3d = nn.Conv2d(self.C_align_1, self.num_anchors, 1)
        self.bbox_z3d = nn.Conv2d(self.C_align_1, self.num_anchors, 1)
        self.bbox_w3d = nn.Conv2d(self.C_align_1, self.num_anchors, 1)
        self.bbox_h3d = nn.Conv2d(self.C_align_1, self.num_anchors, 1)
        self.bbox_l3d = nn.Conv2d(self.C_align_1, self.num_anchors, 1)
        self.bbox_rY3d = nn.Conv2d(self.C_align_1, self.num_anchors, 1)
        ...
        self.align_1_loc = nn.Sequential(
            LocalConv2d(self.num_rows, 512, self.C_align_1, 1),
            nn.ReLU(inplace=True)
        )

        self.align_2_loc = nn.Sequential(
            LocalConv2d(self.num_rows, self.C_align_1, self.C_align_2, 1),
            nn.ReLU(inplace=True)
        )


        # outputs
        self.cls_loc = LocalConv2d(self.num_rows, self.C_align_2, self.num_classes * self.num_anchors, 1, )
        # self.cls_loc = LocalConv2d(self.num_rows, self.prop_feats[0].out_channels, self.num_classes * self.num_anchors, 1, )

        # bbox 2d
        self.bbox_x_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)
        self.bbox_y_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)
        self.bbox_w_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)
        self.bbox_h_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)

        # bbox 3d
        self.bbox_x3d_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)
        self.bbox_y3d_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)
        self.bbox_z3d_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)
        self.bbox_w3d_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)
        self.bbox_h3d_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)
        self.bbox_l3d_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)
        self.bbox_rY3d_loc = LocalConv2d(self.num_rows, self.C_align_1, self.num_anchors, 1)
    ...
    def forward(self, x):
        ...
        # glo分支
        prop_feats_544 = self.align_1(prop_feats)
        glo.set_value('m3drpn_scene_glo_feature',prop_feats_544)
        prop_feats_256 = self.align_2(prop_feats_544)
        glo.set_value('m3drpn_roi_glo_feature',prop_feats_256)

        # cls
        cls = self.cls(prop_feats_256)
        # cls = self.cls(prop_feats)

        # bbox 2d
        bbox_x = self.bbox_x(prop_feats_544)
        bbox_y = self.bbox_y(prop_feats_544)
        bbox_w = self.bbox_w(prop_feats_544)
        bbox_h = self.bbox_h(prop_feats_544)

        # bbox 3d
        bbox_x3d = self.bbox_x3d(prop_feats_544)
        bbox_y3d = self.bbox_y3d(prop_feats_544)
        bbox_z3d = self.bbox_z3d(prop_feats_544)
        bbox_w3d = self.bbox_w3d(prop_feats_544)
        bbox_h3d = self.bbox_h3d(prop_feats_544)
        bbox_l3d = self.bbox_l3d(prop_feats_544)
        bbox_rY3d = self.bbox_rY3d(prop_feats_544)


        # loc分支 
        prop_feats_loc_544 = self.align_1_loc(prop_feats_loc)
        glo.set_value('m3drpn_scene_loc_feature',prop_feats_loc_544)
        prop_feats_loc_256 = self.align_2_loc(prop_feats_loc_544)
        glo.set_value('m3drpn_roi_loc_feature',prop_feats_loc_256)

        # cls
        cls_loc = self.cls_loc(prop_feats_loc_256)
        # cls_loc = self.cls_loc(prop_feats_loc)

        # bbox 2d
        bbox_x_loc = self.bbox_x_loc(prop_feats_loc_544)
        bbox_y_loc = self.bbox_y_loc(prop_feats_loc_544)
        bbox_w_loc = self.bbox_w_loc(prop_feats_loc_544)
        bbox_h_loc = self.bbox_h_loc(prop_feats_loc_544)

        # bbox 3d
        bbox_x3d_loc = self.bbox_x3d_loc(prop_feats_loc_544)
        bbox_y3d_loc = self.bbox_y3d_loc(prop_feats_loc_544)
        bbox_z3d_loc = self.bbox_z3d_loc(prop_feats_loc_544)
        bbox_w3d_loc = self.bbox_w3d_loc(prop_feats_loc_544)
        bbox_h3d_loc = self.bbox_h3d_loc(prop_feats_loc_544)
        bbox_l3d_loc = self.bbox_l3d_loc(prop_feats_loc_544)
        bbox_rY3d_loc = self.bbox_rY3d_loc(prop_feats_loc_544)
